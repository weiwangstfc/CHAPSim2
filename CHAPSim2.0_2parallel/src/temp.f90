...
!------------------------------------------------------------------------------- 
    call transpose_x_to_y(f%qx, qx_ypencil, d%dpcc)                 ! used in x-mom, w   thermal
    call Get_y_midp_C2P_3dArray ( qx_ypencil, d, qx_yppc_ypencil )  ! used in x-mom, w/o thermal
    call transpose_y_to_x(qx_yppc_ypencil, qx_yppc, d%dppc)         ! used in x-mom, wo  thermal

    call transpose_y_to_z(qx_ypencil, qx_zpencil, d%dpcc)           ! used in x-mom, w thermal
    call Get_z_midp_C2P_3dArray ( qx_zpencil, d, qx_zpcp_zpencil )  ! used in x-mon, w/o thermal
    call transpose_z_to_y(qx_zpcp_zpencil, qx_zpcp_ypencil, d%dpcp) ! intermediate, not used.
    call transpose_y_to_x(qx_zpcp_ypencil, qx_zpcp,         d%dpcp) ! used in z-mom, wo  thermal
...
!------------------------------------------------------------------------------- 
    ...
    call transpose_y_to_z(qy_ypencil, qy_zpencil, d%dcpc)           ! used in y-mom, w   thermal
...
!-------------------------------------------------------------------------------
    ...
    call transpose_x_to_y(qz_xpcp,         qz_xpcp_ypencil, d%dpcp) ! intermediate, not used.
    call transpose_y_to_z(qz_xpcp_ypencil, qz_xpcp_zpencil, d%dpcp) ! used in z-mom, wo  thermal

    call transpose_x_to_y(f%qz, qz_ypencil, d%dccp)                   ! used in z-mom, w   thermal
    call transpose_y_to_z(qz_ypencil, qz_zpencil, d%dccp)             ! used in z-mom, w   thermal
    ...
    if(ithermal == 1) then
      ...
      call transpose_z_to_y(qz_zccc_zpencil, qz_zccc_ypencil, d%dccc) ! used in y-mom, w   thermal
      call transpose_y_to_x(qz_zccc_ypencil, qz_zccc,         d%dccc) ! used in x-mom, w   thermal
    end if
    
    if(ithermo == 1) then
      call transpose_x_to_y(f%gx,   gx_ypencil, d%dpcc)
      ...
      call transpose_x_to_y(f%gz,   gz_ypencil, d%dccp)
      call Get_y_midp_C2P_3dArray ( gz_ypencil, d, gz_ycpp_ypencil )
      
      call transpose_x_to_y(f%mVisc, m_ypencil, d%dccc)
      call Get_y_midp_C2P_3dArray (  m_ypencil, d,  m_ycpc_ypencil )

      call transpose_x_to_y(gy_xppc, gy_xppc_ypencil, d%dppc)
      call transpose_x_to_y(qx_xccc, qx_xccc_ypencil, d%dccc)

      call Get_y_midp_P2C_3dArray ( qy_ypencil, d, qy_yccc_ypencil )! used in z-mom, w thermal
      call transpose_y_to_x(qy_yccc_ypencil, qy_yccc, d%dccc) ! used in x-mom, w thermal

      call transpose_y_to_x(gx_yppc_ypencil, gx_yppc, d%dppc)
    end if
    ...
    if(ithermo == 1) then
      call transpose_y_to_z(gx_ypencil, gx_zpencil, d%dpcc)
      call transpose_y_to_z(gy_ypencil, gy_zpencil, d%dcpc)
      call transpose_y_to_z(gz_ypencil, gz_zpencil, d%dccp)
      call transpose_y_to_z( m_ypencil,  m_zpencil, d%dccc)
      ...
      call transpose_z_to_y(gy_zcpp_zpencil, gy_zcpp_ypencil, d%dcpp)

      call Get_z_midp_P2C_3dArray ( qz_zpencil, d, qz_zccc_zpencil )  ! intermediate, not used.
      call transpose_z_to_y(qz_zccc_zpencil, qz_zccc_ypencil, d%dccc) ! used in y-mom, w  thermal
      call transpose_y_to_x(qz_zccc_ypencil, qz_zccc,         d%dccc) ! used in x-mom, wo thermal
...
!-------------------------------------------------------------------------------
      call transpose_z_to_y(m_zccp_zpencil, m_zccp_ypencil, d%dccp)
      call transpose_y_to_x(m_zccp_ypencil, m_zccp,         d%dccp)
      call transpose_y_to_x(m_ycpc_ypencil, m_ycpc,         d%dcpc)
...
!-------------------------------------------------------------------------------
      call transpose_x_to_y(m_xpcc,       m_xpcc_ypencil, d%dpcc)
      call transpose_x_to_y(dmdx_ycpc, dmdx_ycpc_ypencil, d%dcpc)
      ...
      call transpose_y_to_x(div_ypencil,       div0,      d%dccc)
      call transpose_y_to_x(dmdy_xpcc_ypencil, dmdy_xpcc, d%dpcc)
      div = div + div0
!-------------------------------------------------------------------------------
! In Z-pencil, dm_xpcc/dz & dm_ycpc/dz operation in z direction
!-------------------------------------------------------------------------------
      call transpose_y_to_z(m_xpcc_ypencil, m_xpcc_zpencil, d%dpcc)
      call transpose_y_to_z(m_ycpc_ypencil, m_ycpc_zpencil, d%dcpc)
      ...
      call transpose_z_to_y(dmdz_ycpc_zpencil, dmdz_ycpc_ypencil, d%dcpc)

      call transpose_z_to_y(div_zpencil, div_ypencil, d%dccc)
      call transpose_y_to_x(div_ypencil, div0,        d%dccc)
      div = div + div0
