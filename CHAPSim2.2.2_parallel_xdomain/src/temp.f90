...
!------------------------------------------------------------------------------- 
    ...
    call Get_y_midp_C2P_3D ( qx_ypencil, d, qx_yppc_ypencil )  ! used in x-mom, w/o thermal
    ...
    call Get_z_midp_C2P_3D ( qx_zpencil, d, qx_zpcp_zpencil )  ! used in x-mon, w/o thermal
...
!------------------------------------------------------------------------------- 
    ...
    call Get_x_midp_C2P_3D ( f%qy, d, qy_xppc )                ! used in mom-x, w/o thermal
    ...
    call Get_z_midp_C2P_3D ( qy_zpencil, d, qy_zcpp_zpencil )  ! used in y-mon, w   thermal
    ...
    if(ithermo == 1) then
      call Get_x_midp_P2C_3D ( f%qx,    d, qx_xccc )
      call Get_x_midp_P2C_3D ( f%gx,    d, gx_xccc )
      call Get_x_midp_C2P_3D ( f%gy,    d, gy_xppc )
      call Get_x_midp_C2P_3D ( f%gz,    d, gz_xpcp )
      call Get_x_midp_C2P_3D ( f%mVisc, d,  m_xpcc )
    end if
...
!-------------------------------------------------------------------------------
    call Get_x_midp_C2P_3D ( f%qz, d, qz_xpcp )                ! used in z-mom, wo  thermal
    ...
    call Get_y_midp_C2P_3D ( qz_ypencil, d, qz_ycpp_ypencil )    ! used in mom-z, w/o thermal
    if(ithermal == 1) then
      call Get_z_midp_P2C_3D ( qz_zpencil, d, qz_zccc_zpencil )  ! intermediate, not used.
    ...
    if(ithermo == 1) then
      ...
      call Get_y_midp_C2P_3D ( gx_ypencil, d, gx_yppc_ypencil )

      call transpose_x_to_y(f%gy,   gy_ypencil, dm%dcpc)
      call Get_y_midp_P2C_3D ( gy_ypencil, d, gy_yccc_ypencil )

      call transpose_x_to_y(f%gz,   gz_ypencil, dm%dccp)
      call Get_y_midp_C2P_3D ( gz_ypencil, d, gz_ycpp_ypencil )
      
      call transpose_x_to_y(f%mVisc, m_ypencil, dm%dccc)
      call Get_y_midp_C2P_3D (  m_ypencil, d,  m_ycpc_ypencil )

      call transpose_x_to_y(gy_xppc, gy_xppc_ypencil, dm%dppc)
      call transpose_x_to_y(qx_xccc, qx_xccc_ypencil, dm%dccc)

      call Get_y_midp_P2C_3D ( qy_ypencil, d, qy_yccc_ypencil )! used in z-mom, w thermal
    ...
    if(ithermo == 1) then
      ...
      call Get_z_midp_C2P_3D ( gx_zpencil, d, gx_zpcp_zpencil )
      call Get_z_midp_C2P_3D ( gy_zpencil, d, gy_zcpp_zpencil )
      call Get_z_midp_P2C_3D ( gz_zpencil, d, gz_zccc_zpencil )
      c
...
!-------------------------------------------------------------------------------
      ...
      call Get_x_1st_derivative_C2P_3D( f%mVisc, d, dmdx_xpcc )
      call Get_x_1st_derivative_C2C_3D(  m_ycpc, d, dmdx_ycpc )
      call Get_x_1st_derivative_C2C_3D(  m_zccp, d, dmdx_zccp )
      call Get_x_1st_derivative_P2C_3D(    f%qx, d, div0      )
      div = div + div0
!-------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------
      ...
      call Get_y_1st_derivative_C2C_3D( m_xpcc_ypencil, d, dmdy_xpcc_ypencil )
      call Get_y_1st_derivative_C2P_3D( m_ypencil,      d, dmdy_ycpc_ypencil )
      call Get_y_1st_derivative_C2C_3D( m_zccp_ypencil, d, dmdy_zccp_ypencil )
      call Get_y_1st_derivative_P2C_3D( qy_ypencil,     d,       div_ypencil )
...
!-------------------------------------------------------------------------------
      ...
      call Get_z_1st_derivative_C2C_3D( m_xpcc_zpencil, d, dmdz_xpcc_zpencil )
      call Get_z_1st_derivative_C2C_3D( m_ycpc_zpencil, d, dmdz_ycpc_zpencil )
      call Get_z_1st_derivative_C2P_3D(      m_zpencil, d, dmdz_zccp_zpencil )
      call Get_z_1st_derivative_P2C_3D(     qz_zpencil, d,       div_zpencil )
