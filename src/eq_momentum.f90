...
!----------------------------------------------------------------------------------------------------------
    ...
    call Get_z_midp_C2P_3D(qx_zpencil, qx_pcp_zpencil, dm, dm%ibcz_qx(:, IBC_PCC), dm%fbcz_qx) ! qx_pcp_zpencil : x-mom, w+o thermal
...
!                                              | --> qy_ccc_zpencil(WTT)              
!----------------------------------------------------------------------------------------------------------
    call Get_x_midp_C2P_3D(fl%qy, qy_ppc, dm, dm%ibcx_qy(:, IBC_CPC), dm%fbcx_qy) ! qy_ppc : y-mom, w+o thermal
    ...
    call Get_z_midp_C2P_3D(qy_zpencil, qy_cpp_zpencil, dm, dm%ibcz_qy(:, IBC_CPC), dm%fbcz_qy)  ! qy_cpp_zpencil : y-mom, w+o thermal
    call Get_y_midp_P2C_3D(qy_ypencil, qy_ccc_ypencil, dm, dm%ibcy_qy(:, IBC_CPC), dm%fbcy_qy) !
...
!----------------------------------------------------------------------------------------------------------
    !write(*,*) nrank,  'test-3'
    call Get_x_midp_C2P_3D(fl%qz, qz_pcp, dm, dm%ibcx_qz(:, IBC_CCP), dm%fbcx_qz) ! x-pencil : z-mom, w+o   thermal
    call transpose_x_to_y (fl%qz, qz_ypencil, dm%dccp) ! qz_ypencil : z-mom, w+o   thermal
    call Get_y_midp_C2P_3D(qz_ypencil, qz_cpp_ypencil, dm, dm%ibcy_qz(:, IBC_CCP), dm%fbcy_qz) ! qz_cpp_ypencil : z-mom, w+o   thermal
    call transpose_y_to_z (qz_ypencil, qz_zpencil, dm%dccp) ! z-pencil : z-mom, w+o   thermal
    call Get_z_midp_P2C_3D(qz_zpencil, qz_ccc_zpencil, dm, dm%ibcz_qz(:, IBC_CCP), dm%fbcz_qz) ! intermediate, accc_zpencil = qz_ccc_zpencil
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_midp_P2C_3D(fl%gx, gx_ccc, dm, dm%ibcx_qx(:, IBC_PCC), dm%fbcx_gx) ! 
      call transpose_x_to_y (fl%gx,        apcc_ypencil, dm%dpcc)                   ! intermediate, apcc_ypencil = gx_ypencil
      call Get_y_midp_C2P_3D(apcc_ypencil, appc_ypencil, dm, dm%ibcy_qx(:, IBC_PCC), dm%fbcy_gx(:, :, :) ) ! intermediate, appc_ypencil = gx_ppc_ypencil
      ...
      call Get_z_midp_C2P_3D(apcc_zpencil, apcp_zpencil, dm, dm%ibcz_qx(:, IBC_PCC), dm%fbcz_gx(:, :, :) ) ! intermediate, apcp_zpencil = gx_pcp_zpencil
...
!----------------------------------------------------------------------------------------------------------
      ...
      call Get_y_midp_P2C_3D(gy_ypencil, gy_ccc_ypencil, dm, dm%ibcy_qy(:, IBC_CPC), dm%fbcy_gy )     ! 
      call Get_x_midp_C2P_3D(fl%gy,      appc,           dm, dm%ibcx_qy(:, IBC_CPC), dm%fbcx_gy )     ! 
      ...
      call Get_z_midp_C2P_3D(acpc_zpencil, acpp_zpencil, dm, dm%ibcz_qy(:, IBC_CPC), dm%fbcz_gy ) ! intermediate, acpp_zpencil = gy_cpp_zpencil
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_midp_C2P_3D(fl%gz, apcp, dm, dm%ibcx_qz(:, IBC_CCP), dm%fbcx_gz ) ! intermediate, apcp = gz_pcp
      ...
      call Get_z_midp_P2C_3D(gz_zpencil, gz_ccc_zpencil,   dm, dm%ibcz_qz(:, IBC_CCP), dm%fbcz_gz ) ! intermediate, acpp_ypencil = gz_cpp_ypencil

      call Get_y_midp_C2P_3D(accp_ypencil, acpp_ypencil,   dm, dm%ibcy_qz(:, IBC_CCP), dm%fbcy_gz ) ! intermediate, acpp_ypencil = gz_cpp_ypencil
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_1st_derivative_C2P_3D(fl%mVisc, dmdx_pcc, dm, dm%ibcx_Th(:, IBC_CCC), dm%ftpbcx_var(:, :, :)%m )  ! x-pencil : x-mom, w thermal
      ...
      call Get_y_1st_derivative_C2P_3D(accc_ypencil,   dmdy_cpc_ypencil, dm, dm%ibcy_Th(:, IBC_CCC), dm%ftpbcy_var(:, :, :)%m)                ! x-pencil : y-mom, w thermal                                   ! intermediate, accc_ypencil = m_ypencil 
      call Get_y_midp_C2P_3D(accc_ypencil,             m_cpc_ypencil,    dm, dm%ibcy_Th(:, IBC_CCC), dm%ftpbcy_var(:, :, :)%m)              ! y-pencil : y-mom, w thermal
      ...
      call Get_z_1st_derivative_C2P_3D(accc_zpencil, dmdz_ccp_zpencil, dm, dm%ibcz_Th(:, IBC_CCC), dm%ftpbcz_var(:, :, :)%m )   ! z-pencil : z-mom, w thermal
...
!----------------------------------------------------------------------------------------------------------
      ...
      call Get_x_1st_derivative_P2C_3D(fl%qx, accc, dm, dm%ibcx_qx(:, IBC_PCC), dm%fbcx_qx) ! accc = d(qx)/d(x)_ccc
      div = div + accc ! = d(qx)/d(x)_ccc

      call Get_y_1st_derivative_P2C_3D(qy_ypencil, accc_ypencil, dm, dm%ibcy_qy(:, IBC_CPC)) ! accc_ypencil = d(qy)/(y)_ccc_ypencil
      call transpose_y_to_x (accc_ypencil, accc,         dm%dccc)                                   ! accc = d(qy)/d(y)_ccc
      div = div + accc ! = d(qx)/d(x)_ccc + d(qy)/d(y)_ccc

      call Get_z_1st_derivative_P2C_3D(qz_zpencil, accc_zpencil, dm, dm%ibcz_qz(:, IBC_CCP)) ! accc_zpencil = d(qz)/(z)_ccc_zpencil
...
!----------------------------------------------------------------------------------------------------------  
    ...
    if ( .not. dm%is_thermo) then
      call Get_x_1st_derivative_C2P_3D(-qx_ccc * qx_ccc, apcc, dm, nbc(:, 1), -dm%fbcx_qx * dm%fbcx_qx)
    else
      call Get_x_1st_derivative_C2P_3D(-gx_ccc * qx_ccc, apcc, dm, nbc(:, 1), -dm%fbcx_gx * dm%fbcx_qx)
    end if
    fl%mx_rhs = fl%mx_rhs + apcc
!----------------------------------------------------------------------------------------------------------
! Y-pencil : X-mom convection term (x-c2/3): -d(<gy>^x * <qx>^y)/dy at (i', j, k)
!----------------------------------------------------------------------------------------------------------
    call get_ibc_for_calcuation((dm%ibcy_qy(:, IBC_PPC), nbc, dm%ibcy_qx(:, IBC_PPC)))
    if ( .not. dm%is_thermo) then
      call Get_y_1st_derivative_P2C_3D(-qy_ppc_ypencil * qx_ppc_ypencil, apcc_ypencil, dm, nbc(:, 1))
    else
      call Get_y_1st_derivative_P2C_3D(-gy_ppc_ypencil * qx_ppc_ypencil, apcc_ypencil, dm, nbc(:, 1))
    end if
    mx_rhs_ypencil = mx_rhs_ypencil + apcc_ypencil
!----------------------------------------------------------------------------------------------------------
! Z-pencil : X-mom convection term (x-c3/3): -d(<gz>^x * <qx>^z)/dz at (i', j, k)
!----------------------------------------------------------------------------------------------------------
    call get_ibc_for_calcuation((dm%ibcz_qz(:, IBC_PCP), nbc, dm%ibcz_qx(:, IBC_PCP)))
    if ( .not. dm%is_thermo) then
      call Get_z_1st_derivative_P2C_3D(-qz_pcp_zpencil * qx_pcp_zpencil, apcc_zpencil, dm, nbc(:, 1) )
    else
      call Get_z_1st_derivative_P2C_3D(-gz_pcp_zpencil * qx_pcp_zpencil, apcc_zpencil, dm, nbc(:, 1) )
    end if
...
!----------------------------------------------------------------------------------------------------------
if(ipressure)then
    call Get_x_1st_derivative_C2P_3D( -fl%pres, apcc, dm, dm%ibcx_pr(:, IBC_CCC), -dm%fbcx_pr(:, :, :) )
...
if(iviscous) then
    ...
    call Get_x_1st_derivative_C2P_3D(accc,  apcc, dm, nbc(:, 2)) ! accc = d(du/dx)/dx, at (i', j, k)
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_ibc_for_calcuation(dm%ibcy_qx(:, IBC_PPC), nbc) 
    call Get_y_1st_derivative_P2C_3D(appc_ypencil, apcc_ypencil, dm, nbc(:, 2)) ! d(du/dy)/dy, at (i', j, k)
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_ibc_for_calcuation(dm%ibcz_qx(:, IBC_PCP), nbc) 
    call Get_z_1st_derivative_P2C_3D(apcp_zpencil, apcc_zpencil, dm, nbc(:, 2)) ! d(du/dz)/dz at (i', j, k)
...
!----------------------------------------------------------------------------------------------------------
      if(fl%igravity == i .or. fl%igravity == -i)  then
        call Get_x_midp_C2P_3D(fl%dDens, apcc, dm, dm%ibcx_Th(:, IBC_CCC), dm%ftpbcx_var(:, :, :)%d )
...
!----------------------------------------------------------------------------------------------------------
      call get_ibc_for_calcuation(dm%ibcx_qx(:, IBC_CCC), nbc) 
      call Get_x_1st_derivative_C2P_3D(div, apcc, dm, nbc(:, 2)) ! apcc = d(div)/dx at (i', j, k)
      fl%mx_rhs = fl%mx_rhs + one_third_rre * m_pcc * apcc
!----------------------------------------------------------------------------------------------------------
!   X-pencil : X-mom diffusion term (x-v3/7), -2/3 * d\mu/dx * (div(u)^x) +  
!                                                2 * d\mu/dx * du/dx
!----------------------------------------------------------------------------------------------------------
      call Get_x_midp_C2P_3D (div, apcc, dm, nbc(:, 2)) ! div at (i', j, k), (to add, check: bc is required only if Neumann BC of velocity.)
      fl%mx_rhs = fl%mx_rhs - two_third_rre * dmdx_pcc * apcc
      call Get_x_1st_derivative_P2P_3D(fl%qx, apcc, dm, dm%ibcx_qx(:, IBC_PCC), dm%fbcx_qx(:, :, :) ) ! apcc = d(qx)/dx at (i', j, k)
      fl%mx_rhs = fl%mx_rhs + two_rre       * dmdx_pcc * apcc
!----------------------------------------------------------------------------------------------------------
!   X-pencil : X-mom diffusion term (x-v4/7), d(mu^x)/dy * d(qy^y)/dx at (i', j, k)
!----------------------------------------------------------------------------------------------------------
      call Get_x_1st_derivative_C2P_3D(qy_ccc, apcc, dm, dm%ibcx_qy(:, IBC_CCC), dm%fbcx_qy(:, :, :) ) !apcc = d(qy)/dx
      fl%mx_rhs =  fl%mx_rhs + fl%rre * dmdy_pcc * apcc
!----------------------------------------------------------------------------------------------------------
!   Y-pencil : X-mom diffusion term (x-v5/7), d(mu^x)/dy * d(qx)/dy at (i', j, k)
!----------------------------------------------------------------------------------------------------------
      call Get_y_1st_derivative_C2C_3D(qx_ypencil, apcc_ypencil, dm, dm%ibcy_qx(:, IBC_PCC), dm%fbcy_qx) !apcc_ypencil = d(qx)/dy_ypencil
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_1st_derivative_C2P_3D(qz_ccc, apcc, dm, dm%ibcx_qz(:, IBC_CCC), dm%fbcx_qz(:, :, :) ) ! apcc = d(qz)/dx
      fl%mx_rhs =  fl%mx_rhs + fl%rre * dmdz_pcc * apcc
!----------------------------------------------------------------------------------------------------------
!   Z-pencil : X-mom diffusion term (x-v7/7), d(mu^x)/dz * d(qx)/dz at (i', j, k)
!----------------------------------------------------------------------------------------------------------
      call Get_z_1st_derivative_C2C_3D(qx_zpencil, apcc_zpencil, dm, dm%ibcz_qx(:, IBC_PCC), dm%fbcz_qx) ! apcc_zpencil = d(qx)/dz
...
!----------------------------------------------------------------------------------------------------------
    call get_ibc_for_calcuation(dm%ibcx_qx(:, IBC_PPC), nbc, dm%ibcx_qy(:, IBC_PPC))
    if ( .not. dm%is_thermo) then
      call Get_x_1st_derivative_P2C_3D( -qx_ppc * qy_ppc, acpc, dm, nbc(:, 1))
    else
      call Get_x_1st_derivative_P2C_3D( -gx_ppc * qy_ppc, acpc, dm, nbc(:, 1))
    end if
...
!----------------------------------------------------------------------------------------------------------
    ...
    if ( .not. dm%is_thermo) then
      call Get_y_1st_derivative_C2P_3D(-qy_ccc_ypencil * qy_ccc_ypencil, acpc_ypencil, dm, nbc(:, 1), dm%fbcy_qy * dm%fbcy_qy)
    else
      call Get_y_1st_derivative_C2P_3D(-gy_ccc_ypencil * qy_ccc_ypencil, acpc_ypencil, dm, nbc(:, 1), dm%fbcy_qy * dm%fbcy_qy)
    end if
...
!----------------------------------------------------------------------------------------------------------
    call get_ibc_for_calcuation(dm%ibcz_qz(:, IBC_CPP), nbc, dm%ibcz_qy(:, IBC_CPP))
    if ( .not. dm%is_thermo) then
      call Get_z_1st_derivative_P2C_3D( -qz_cpp_zpencil * qy_cpp_zpencil, acpc_zpencil, dm, nbc(:, 1) )
    else
      call Get_z_1st_derivative_P2C_3D( -gz_cpp_zpencil * qy_cpp_zpencil, acpc_zpencil, dm, nbc(:, 1) )
    end if
...
!----------------------------------------------------------------------------------------------------------
    call Get_y_1st_derivative_C2P_3D( -pres_ypencil, acpc_ypencil, dm, dm%ibcy_pr(:, IBC_CCC), -dm%fbcy_pr(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_ibc_for_calcuation(dm%ibcx_qy(:, IBC_PPC), nbc)
    call Get_x_1st_derivative_P2C_3D(appc, acpc, dm, nbc(:, 2))
    if (.not. dm%is_thermo) then
...
!----------------------------------------------------------------------------------------------------------
    ...
    call Get_y_1st_derivative_C2P_3D(accc_ypencil, acpc_ypencil, dm, nbc(:, 2))
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_ibc_for_calcuation(dm%ibcz_qy(:, IBC_CPP), nbc)
    call Get_z_1st_derivative_P2C_3D(acpp_zpencil, acpc_zpencil, dm, nbc(:, 2))
...
!----------------------------------------------------------------------------------------------------------
      if(fl%igravity == i .or. fl%igravity == -i) then
        call Get_y_midp_C2P_3D(dDens_ypencil, acpc_ypencil, dm, dm%ibcy_Th(:, IBC_CCC), dm%ftpbcy_var(:, :, :)%d )
...
!----------------------------------------------------------------------------------------------------------
      call get_ibc_for_calcuation(dm%ibcy_qx(:, IBC_PCC), nbc)
      call Get_y_1st_derivative_C2P_3D(div_ypencil, acpc_ypencil, dm, nbc(:, 2))  ! to check if it needs bc of div
...
!----------------------------------------------------------------------------------------------------------
      !fbcy = ZERO ! check
      call get_ibc_for_calcuation(dm%ibcy_qx(:, IBC_PCC), nbc)
      call Get_y_midp_C2P_3D (div_ypencil, acpc_ypencil, dm, nbc(:, 2), fbcy )  ! to check if it needs bc of div
...
!----------------------------------------------------------------------------------------------------------
      call Get_y_1st_derivative_C2P_3D(qx_ccc_ypencil, acpc_ypencil, dm, dm%ibcy_qx(:, IBC_CCC), dm%fbcy_qx(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_1st_derivative_C2C_3D(fl%qy, acpc, dm, dm%ibcx_qy(:, IBC_CPC), dm%fbcx_qy)
      fl%my_rhs =  fl%my_rhs + fl%rre * dmdx_cpc * acpc
!----------------------------------------------------------------------------------------------------------
! Y-pencil : Y-mom diffusion term (y-v6/7), d(mu^y)/dz * d(qz^z)/dy at (i, j', k)
!----------------------------------------------------------------------------------------------------------
      call Get_y_1st_derivative_C2P_3D(qz_ccc_ypencil, acpc_ypencil, dm, dm%ibcy_qz(:, IBC_CCC), dm%fbcy_qz(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
      call Get_z_1st_derivative_C2C_3D(qy_zpencil, acpc_zpencil, dm, dm%ibcz_qy(:, IBC_CPC), dm%fbcz_qy)
...
!----------------------------------------------------------------------------------------------------------
    ...
    if(dm%ibcy_qy(1, IBC_CPC) ==  IBC_DIRICHLET .and. dm%dcpc%yst(2) == 1)   then
      acpc_ypencil(:, 1, :) = ZERO 
      my_rhs_pfc_ypencil(:, 1, :) = ZERO 
    end if
    if(dm%ibcy_qy(2, IBC_CPC) ==  IBC_DIRICHLET .and. dm%dcpc%yen(2) == dm%dcpc%ysz(2)) then
...
!----------------------------------------------------------------------------------------------------------
    call get_ibc_for_calcuation(dm%ibcx_qx(:, IBC_PCP), nbc, dm%ibcx_qz(:, IBC_PCP))
    if ( .not. dm%is_thermo) then
      call Get_x_1st_derivative_P2C_3D( -qx_pcp * qz_pcp, accp, dm, nbc(:, 1))
    else
      call Get_x_1st_derivative_P2C_3D( -gx_pcp * qz_pcp, accp, dm, nbc(:, 1))
    end if
    fl%mz_rhs = fl%mz_rhs + accp
!----------------------------------------------------------------------------------------------------------
! Y-pencil : Z-mom convection term (z-c2/3), d(gy^z * qz^y)/dy at (i, j, k')
!----------------------------------------------------------------------------------------------------------
    call get_ibc_for_calcuation(dm%ibcy_qy(:, IBC_CPP), nbc, dm%ibcy_qz(:, IBC_CPP))
    if ( .not. dm%is_thermo) then
      call Get_y_1st_derivative_P2C_3D( -qy_cpp_ypencil * qz_cpp_ypencil, accp_ypencil, dm, nbc(:, 1))
   else
      call Get_y_1st_derivative_P2C_3D( -gy_cpp_ypencil * qz_cpp_ypencil, accp_ypencil, dm, nbc(:, 1))
    end if
    mz_rhs_ypencil = mz_rhs_ypencil + accp_ypencil
!----------------------------------------------------------------------------------------------------------
! Z-pencil : Z-mom convection term (y-c3/3), d(gz * qz)/dz at (i, j, k')
!----------------------------------------------------------------------------------------------------------
    call get_ibc_for_calcuation(dm%ibcz_qz(:, IBC_CCC), nbc, dm%ibcz_qz(:, IBC_CCC))
    if ( .not. dm%is_thermo) then
      call Get_z_1st_derivative_C2P_3D(-qz_ccc_zpencil * qz_ccc_zpencil, accp_zpencil, dm, nbc(:, 1))
    else
      call Get_z_1st_derivative_C2P_3D(-gz_ccc_zpencil * qz_ccc_zpencil, accp_zpencil, dm, nbc(:, 1))
    end if
...
!----------------------------------------------------------------------------------------------------------
    call Get_z_1st_derivative_C2P_3D( -pres_zpencil, accp_zpencil, dm, dm%ibcz_pr(:, IBC_CCC), -dm%fbcz_pr(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_ibc_for_calcuation(dm%ibcx_qz(:, IBC_PCP), nbc)
    call Get_x_1st_derivative_P2C_3D(apcp,  accp, dm, nbc(:, 2))
    if ( .not. dm%is_thermo) then
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_ibc_for_calcuation(dm%ibcy_qz(:, IBC_CPP), nbc)
    call Get_y_1st_derivative_P2C_3D( acpp_ypencil, accp_ypencil, dm, nbc(:, 2))
    if ( .not. dm%is_thermo) then
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_ibc_for_calcuation(dm%ibcz_qz(:, IBC_CCC), nbc)
    call Get_z_1st_derivative_C2P_3D(accc_zpencil, accp_zpencil, dm, nbc(:, 2))
...
!----------------------------------------------------------------------------------------------------------
      if( fl%igravity == i .or. fl%igravity == -i) then
        call Get_z_midp_C2P_3D(dDens_zpencil, accp_zpencil, dm, dm%ibcz_Th(:, IBC_CCC), dm%ftpbcz_var(:, :, :)%d )
...
!----------------------------------------------------------------------------------------------------------
      fbcz = ZERO
      call get_ibc_for_calcuation(dm%ibcz_qz(:, IBC_CCP), nbc)
      call Get_z_1st_derivative_C2P_3D(div_zpencil, accp_zpencil, dm, nbc(:, 2), fbcz )
...
!----------------------------------------------------------------------------------------------------------
      fbcz = ZERO
      call get_ibc_for_calcuation(dm%ibcz_qz(:, IBC_CCP), nbc)
      call Get_z_midp_C2P_3D(div_zpencil, accp_zpencil, dm,  nbc(:, 2), fbcz )
...
!----------------------------------------------------------------------------------------------------------
      call Get_z_1st_derivative_C2P_3D(qx_ccc_zpencil, accp_zpencil, dm, dm%ibcz_qx(:, IBC_CCC), dm%fbcz_qx(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_1st_derivative_C2C_3D(fl%qz, accp, dm, dm%ibcx_qz(:, IBC_CCP), dm%fbcx_qz)
      fl%mz_rhs =  fl%mz_rhs + fl%rre * dmdx_ccp * accp
!----------------------------------------------------------------------------------------------------------
! Z-pencil : Z-mom diffusion term (z-v6/7), d(mu^z)/dy * d(qy^y)/dz at (i, j, k')
!----------------------------------------------------------------------------------------------------------
      call Get_z_1st_derivative_C2P_3D(qy_ccc_zpencil, accp_zpencil, dm, dm%ibcz_qy(:, IBC_CCC), dm%fbcz_qy(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
      call Get_y_1st_derivative_C2C_3D(qz_ypencil, accp_ypencil, dm, dm%ibcy_qz(:, IBC_CCP), dm%fbcy_qz)
...
!   x-pencil : flow drive terms (source terms) in periodic Streamwise flow
    if (fl%idriven == IDRVF_X_MASSFLUX) then
      call Get_volumetric_average_3d(.false., dm%ibcy_qx(:, IBC_PCC), dm%fbcy_qx(:, :, :), dm, dm%dpcc, fl%mx_rhs, rhsx_bulk, "mx_rhs")
...
!   x-pencil : flow drive terms (source terms) in periodic Streamwise flow
    if (fl%idriven == IDRVF_Z_MASSFLUX) then
      call Get_volumetric_average_3d(.false., dm%ibcy_qz(:, IBC_CCP), dm%fbcy_qz(:, :, :), dm, dm%dccp, fl%mz_rhs, rhsz_bulk, "mz_rhs")
...
!----------------------------------------------------------------------------------------------------------
    dphidx_pcc = ZERO
    call Get_x_1st_derivative_C2P_3D(phi_ccc,  dphidx_pcc, dm, dm%ibcx_pr(:, IBC_CCC), dm%fbcx_pr(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
    ...
    call Get_y_1st_derivative_C2P_3D(phi_ccc_ypencil, dphidy_cpc_ypencil, dm, dm%ibcy_pr(:, IBC_CCC), dm%fbcy_pr(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
    ...
    call Get_z_1st_derivative_C2P_3D(pphi_ccc_zpencil, dphidz_ccp_zpencil, dm, dm%ibcz_pr(:, IBC_CCC), dm%fbcz_pr(:, :, :) )
