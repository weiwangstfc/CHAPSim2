...
!> \param[in]     isub          the RK iteration to get correct Coefficient 
!_______________________________________________________________________________
  subroutine Compute_momentum_rhs(fl, dm, isub, tm)
    use parameters_constant_mod
    use udf_type_mod
    use operations
...
!==========================================================================================================
    ...
    if(dm%is_thermo) then
      if(.not. present(tm)) call Print_error_msg("Input error in Compute_momentum_rhs.")
    end if
!==========================================================================================================
...
!BCx:| -[1dx]-> qxdx_pcc_xpencil (temp) -> qxdx_4cc_xpencil
!    | -[x2y]-> qx_ypencil(temp) 
!               | -[ipy]-> qxiy_ppc_ypencil(common) -[y2x]-> qxiy_ppc_xpencil(no thermal)
!                                                         BCy:|-[1dx]->qxdx_cpc_xpencil-[x2y]->qxdx_cpc_ypencil->qxdx_c4c_ypencil 
...
!                                                                                                   BCz:|-[1dx]->qxdx_ccp_xpencil-[x2y2z]->qxdx_ccp_zpencil->qxdx_cc4_zpencil 
...
!----------------------------------------------------------------------------------------------------------
    ...
    if( any(dm%ibcx(:, 1) == IBC_DIRICHLET) ) then ! for Dirchilet x-bc of qxdx_4cc_xpencil
      call Get_x_1st_derivative_P2P_3D(fl%qx, apcc_xpencil, dm, dm%ibcx(:, 1), fl%fbcx_qx(:, :, :))
      call get_dirichlet_geo_bcx(dm%ibcx(:, 1), apcc_xpencil, qxdx_4cc_xpencil)
    end if
    ...
    if( any(dm%ibcy(:, 1) == IBC_DIRICHLET) ) then  ! for Dirchilet y-bc of qxdx_c4c_ypencil
      if(.not. dm%is_thermo) then
        appc_xpencil = qxiy_ppc_xpencil
      else
        call transpose_y_to_x (qxiy_ppc_ypencil, appc_xpencil, dm%dppc) ! appc_xpencil = qxiy_ppc_xpencil
      end if
      call Get_x_1st_derivative_P2C_3D(appc_xpencil, acpc_xpencil, dm, dm%ibcx(:, 1), fl%fbcx_qx(:, :, :))
      call transpose_x_to_y(acpc_xpencil, acpc_ypencil, dm%dcpc)
      call get_dirichlet_geo_bcy(dm%ibcy(:, 1), acpc_ypencil, qxdx_c4c_ypencil)
    end if
    ...
    if(any(dm%ibcz(:, 1) == IBC_DIRICHLET)) then  ! for Dirchilet z-bc of qxdx_cc4_zpencil
      if(.not. dm%is_thermo) then
        apcp_xpencil = qxiz_pcp_xpencil
      else
        call transpose_z_to_y (qxiz_pcp_zpencil, apcp_ypencil, dm%dpcc)! apcp_ypencil = qxiz_pcp_ypencil
        call transpose_y_to_x (apcp_ypencil, apcp_xpencil, dm%dpcp) ! apcp_ypencil = qxiz_pcp_ypencil
      end if
      call Get_x_1st_derivative_P2C_3D(apcp_xpencil, accp_xpencil, dm, dm%ibcx(:, 1), fl%fbcx_qx(:, :, :))
      call transpose_x_to_y(accp_xpencil, accp_ypencil, dm%dccp)
      call transpose_y_to_z(accp_ypencil, accp_zpencil, dm%dccp)
      call get_dirichlet_geo_bcz(dm%ibcz(:, 1), accp_zpencil, qxdx_cc4_zpencil)
    end if
...
!           BCx:|-[x2y]->qyix_ppc_ypencil-[1dy]-> qydy_pcc_ypencil -[y2x]-> qydy_pcc_xpencil -> qydy_4cc_xpencil(BCx)
...
!                      
!           BCy:| -[1dy]-> qydy_cpc_ypencil(no-cly, temp) -> qydy_c4c_ypencil(no-cly, BC)
!               | -[y2z]-> qy_zpencil(temp) 
!                          | -[ipz]-> qyiz_cpp_zpencil(no-cly) -[z2y]-> qyiz_cpp_ypencil(no-thermal)    
!                                 BCz:| -[z2y]-> qy_cpp_ypencil-[1dy]-> qydy_ccp_ypencil-[y2z]-> qydy_ccp_zpencil -> qydy_cc4_zpencil    
...
!----------------------------------------------------------------------------------------------------------
    ...
    if( any(dm%ibcx(:, 2) == IBC_DIRICHLET) ) then ! for Dirchilet x-bc of qydy_4cc_xpencil
      if(.not. dm%is_thermo) then
        appc_ypencil = qyix_ppc_ypencil
      else
        call transpose_x_to_y (qyix_ppc_xpencil, appc_ypencil, dm%dppc)
      end if
      call Get_y_1st_derivative_P2C_3D(appc_ypencil, apcc_ypencil, dm, dm%ibcy(:, 2), fl%fbcy_qy(:, :, :))
      call transpose_y_to_x(apcc_ypencil, apcc_xpencil, dm%dpcc)
      call get_dirichlet_geo_bcx(dm%ibcx(:, 2), apcc_xpencil, qydy_4cc_xpencil)
    end if
    ...
    if( any(dm%ibcy(:, 2) == IBC_DIRICHLET) ) then ! for Dirchilet y-bc of qydy_c4c_ypencil
        call Get_y_1st_derivative_P2P_3D(acpc_ypencil, acpc_ypencil1, dm, dm%ibcy(:, 2), fl%fbcy_qy(:, :, :))
        call get_dirichlet_geo_bcy(dm%ibcy(:, 2), acpc_ypencil1, qydy_c4c_ypencil)
      end if
    ...
    if( any(dm%ibcz(:, 2) == IBC_DIRICHLET) ) then ! for Dirchilet z-bc of qydy_cc4_zpencil
      call Get_z_midp_C2P_3D(acpc_zpencil, acpp_zpencil, dm, dm%ibcz(:, 2), fl%fbcz_qy(:, :, :) )
      call transpose_z_to_y(acpp_zpencil, acpp_ypencil, dm%dcpp)
      call Get_y_1st_derivative_P2C_3D(acpp_ypencil, accp_ypencil, dm, dm%ibcy(:, 2), fl%fbcy_qy(:, :, :))
      call transpose_y_to_z(accp_ypencil, accp_zpencil, dm%dccp)
      call get_dirichlet_geo_bcz(dm%ibcz(:, 2), accp_zpencil, qydy_cc4_zpencil)
    end if
!----------------------------------------------------------------------------------------------------------
!    qz 
...
!               BCx:|-[x2y2z]->qzix_pcp_zpencil-[1dz]-> qzdz_pcc_zpencil -[z2y2x]-> qzdz_pcc_xpencil -> qzdz_4cc_xpencil(BCx)
!    | -[x2y]-> qz_ypencil(temp) 
...
!           BCy:| -[ipy]-> qziy_cpp_ypencil -[y2z]-> qziy_cpp_zpencil -[1dz]-> qzdz_cpc_zpencil -[z2y]-> qzdz_cpc_ypencil-> qzdz_c4c_ypencil
!               | -[y2z]-> qz_zpencil(temp) 
!                          | -[ipz]-> qziz_ccc_zpencil(common)
!                          | -[1dz]-> qzdz_ccc_zpencil(common) <need BCz> -[y2z]-> qzdz_ccc_ypencil(cly-only) <need BCy>
!                      BCz:| -[1dz]-> qzdz_ccp_zpencil -> qzdz_cc4_zpencil
!----------------------------------------------------------------------------------------------------------
    ...
    if( any(dm%ibcx(:, 3) == IBC_DIRICHLET) ) then ! for Dirchilet x-bc of qzdz_4cc_xpencil
      if(.not. dm%is_thermo) then
      ...
      call Get_z_1st_derivative_P2C_3D(apcp_zpencil, apcc_zpencil, dm, dm%ibcz(:, 3), fl%fbcz_qz(:, :, :))
      call transpose_z_to_y(apcc_zpencil, apcc_ypencil, dm%dpcc)
      call transpose_y_to_x(apcc_ypencil, apcc_xpencil, dm%dpcc)
      call get_dirichlet_geo_bcx(dm%ibcx(:, 3), apcc_xpencil, qzdz_4cc_xpencil)
    end if
    ...
    if( any(dm%ibcy(:, 3) == IBC_DIRICHLET) ) then ! for Dirchilet y-bc of qzdz_c4c_ypencil
      call Get_y_midp_C2P_3D(accp_ypencil, acpp_ypencil, dm, dm%ibcy(:, 3), fl%fbcy_qz(:, :, :) )
      call transpose_y_to_z(acpp_ypencil, acpp_zpencil, dm%dcpp)
      call Get_z_1st_derivative_P2C_3D(acpp_zpencil, acpc_zpencil, dm, dm%ibcz(:, 3), fl%fbcz_qz(:, :, :))
      call transpose_z_to_y(acpc_zpencil, acpc_ypencil, dm%dcpc)
      call get_dirichlet_geo_bcy(dm%ibcx(:, 3), acpc_ypencil, qzdz_c4c_ypencil)
    end if
    ...
    if( any(dm%ibcz(:, 3) == IBC_DIRICHLET) ) then ! for Dirchilet z-bc of qzdz_cc4_zpencil
      call Get_z_1st_derivative_P2P_3D(accp_zpencil, accp_zpencil1, dm, dm%ibcz(:, 3), fl%fbcz_qz(:, :, :))
      call get_dirichlet_geo_bcz(dm%ibcz(:, 3), accp_zpencil1, qzdz_cc4_zpencil)
    end if
...
!==========================================================================================================
    ...
    div_ccc_xpencil = div_ccc_xpencil + accc_xpencil  ! div_tmp = du/dx + dv/dy
    ...
    call transpose_y_to_x (accc_ypencil, accc_xpencil, dm%dccc)   
    div_ccc_xpencil = div_ccc_xpencil + accc_xpencil   ! div_tmp = du/dx + dv/dy + dw/dz
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_midp_P2C_3D(fl%gx, gxix_ccc_xpencil, dm, dm%ibcx(:, 1), fl%fbcx_gx(:, :, :))
      
      call transpose_x_to_y (fl%gx, apcc_ypencil, dm%dpcc) !gx_ypencil
      call Get_y_midp_C2P_3D(apcc_ypencil, appc_ypencil, dm, dm%ibcy(:, 1), fl%fbcy_gx(:, :, :))
      ...
      call Get_z_midp_C2P_3D(apcc_zpencil, apcp_zpencil, dm, dm%ibcz(:, 1), fl%fbcz_gx(:, :, :))
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_midp_C2P_3D(fl%gy, appc_xpencil, dm, dm%ibcx(:, 2), fl%fbcx_gy(:, :, :))
      call transpose_x_to_y (appc_xpencil, gyix_ppc_ypencil, dm%dppc)

      call transpose_x_to_y (fl%gy, acpc_ypencil, dm%dcpc) !gy_ypencil
      call Get_y_midp_P2C_3D(acpc_ypencil, gyiy_ccc_ypencil, dm, dm%ibcy(:, 2), fl%fbcy_gy(:, :, :) )

      call transpose_y_to_z(acpc_ypencil, acpc_zpencil, dm%dcpc)!qy_zpencil
      call Get_z_midp_C2P_3D(acpc_zpencil, acpp_zpencil, dm, dm%ibcz(:, 2), fl%fbcz_gy(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
      call Get_x_midp_C2P_3D(fl%gz, apcp_xpencil, dm, dm%ibcx(:, 3), fl%fbcx_gz(:, :, :))
      ...
      call Get_z_midp_P2C_3D(accp_zpencil, gziz_ccc_zpencil, dm, dm%ibcz(:, 3), fl%fbcz_gz(:, :, :))
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------
      ...
      if( any(dm%ibcz(:, 3) == IBC_DIRICHLET) ) then
        call Get_z_midp_C2P_3D(mu_ccc_zpencil, accp_zpencil, dm, dm%ibcx(:, 5), tm%fbcx_ftp(:, :, :)%m)
        call get_dirichlet_geo_bcz(dm%ibcz(:, 5), accp_zpencil, mu_cc4_zpencil)
      end if

      call Get_x_midp_C2P_3D(mu_ccc_xpencil, apcc_xpencil, dm, dm%ibcx(:, 5), tm%fbcx_ftp(:, :, :)%m) ! muix_pcc_xpencil
      if( any(dm%ibcx(:, 1) == IBC_DIRICHLET) ) then ! x direction momentum need
        call get_dirichlet_geo_bcx(dm%ibcx(:, 5), apcc_xpencil, mu_4cc_xpencil)
      end if
      ...
      call Get_y_midp_C2P_3D(mu_ccc_ypencil, acpc_ypencil, dm, dm%ibcy(:, 5), tm%fbcy_ftp(:, :, :)%m) ! muiy_cpc_ypencil
      if( any(dm%ibcy(:, 2) == IBC_DIRICHLET) ) then
        call get_dirichlet_geo_bcy(dm%ibcy(:, 5), acpc_ypencil, mu_c4c_ypencil)
      end if
...
!----------------------------------------------------------------------------------------------------------
      ...
      if( any(dm%ibcy(:, 2) == IBC_DIRICHLET) )then
        call Get_y_1st_derivative_P2P_3D(acpc_ypencil, acpc_ypencil1, dm, dm%ibcy(:, 2), fl%fbcy_qyr(:, :, :))
        call get_dirichlet_geo_bcy(dm%ibcy(:, 2), acpc_ypencil1, qyrdy_c4c_ypencil)
      end if
...
!               | -[y2z]-> qzr_zpencil(temp) -[ipz]-> qzriz_ccc_zpencil       
!----------------------------------------------------------------------------------------------------------
      accp_xpencil = fl%qz
      ...
      call Get_y_midp_C2P_3D(accp_ypencil, qzriy_cpp_ypencil, dm, dm%ibcy(:, 3), fl%fbcy_qzr(:, :, :))
      call transpose_y_to_z(qzriy_cpp_ypencil, qzriy_cpp_zpencil, dm%dcpp)

      call Get_y_1st_derivative_C2P_3D(accp_ypencil, qzrdy_cpp_ypencil, dm, dm%ibcy(:, 3), fl%fbcy_qzr(:, :, :))
...
!----------------------------------------------------------------------------------------------------------
        ...
        call transpose_x_to_y(acpc_xpencil, acpc_ypencil, dm%dcpc)
        call transpose_y_to_z(acpc_ypencil, acpc_zpencil, dm%dcpc)
        call Get_z_midp_C2P_3D(acpc_zpencil, acpp_zpencil, dm, dm%ibcz(:, 3), fl%fbcz_gyr(:, :, :))
...
!----------------------------------------------------------------------------------------------------------
        ...
        call Get_y_midp_C2P_3D(accp_ypencil, acpp_ypencil, dm, dm%ibcy(:, 3), fl%fbcy_gzr(:, :, :))
        call transpose_y_to_z(acpp_ypencil, gzriy_cpp_zpencil, dm%dccp)

        call transpose_y_to_z(accp_ypencil, accp_zpencil, dm%dccp)
        call Get_z_midp_P2C_3D(accp_zpencil, accc_zpencil, dm, dm%ibcz(:, 3), fl%fbcz_gzr(:, :, :))
...
!----------------------------------------------------------------------------------------------------------  
    if ( .not. dm%is_thermo) then
      ...
      call get_dirichlet_geo_bcx(dm%ibcx(:, i), fl%fbcx_qx * fl%fbcx_qx, fbcx_4cc)
    else
      accc_xpencil = gxix_ccc_xpencil
      call get_dirichlet_geo_bcx(dm%ibcx(:, i), fl%fbcx_gx * fl%fbcx_qx, fbcx_4cc)
    end if
    
    accc_xpencil =  accc_xpencil * qxix_ccc_xpencil
    call Get_x_1st_derivative_C2P_3D( accc_xpencil, apcc_xpencil, dm, dm%ibcx(:, i), fbcx_4cc )
...
!----------------------------------------------------------------------------------------------------------
    if ( .not. dm%is_thermo) then
      appc_ypencil = qyix_ppc_ypencil
      call get_dirichlet_geo_bcy(dm%ibcy(:, i), fl%fbcy_qy * fl%fbcy_qx, fbcy_p4c)
    else
      appc_ypencil = gyix_ppc_ypencil
      call get_dirichlet_geo_bcy(dm%ibcy(:, i), fl%fbcy_gy * fl%fbcy_qx, fbcy_p4c)
    end if
    
    appc_ypencil = appc_ypencil * qxiy_ppc_ypencil
    call Get_y_1st_derivative_P2C_3D( appc_ypencil, apcc_ypencil, dm, dm%ibcy(:, i), fbcy_p4c )
...
!----------------------------------------------------------------------------------------------------------
    if ( .not. dm%is_thermo) then
      apcp_zpencil = qzix_pcp_zpencil
      call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_qz * fl%fbcz_qx, fbcz_pc4)
    else
      apcp_zpencil = gzix_pcp_zpencil
      call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_gz * fl%fbcz_qx, fbcz_pc4)
    end if

    apcp_zpencil = apcp_zpencil * qxiz_pcp_zpencil
    call Get_z_1st_derivative_P2C_3D( apcp_zpencil, apcc_zpencil, dm, dm%ibcz(:, i), fbcz_pc4)
...
!----------------------------------------------------------------------------------------------------------
    call Get_x_1st_derivative_C2P_3D( fl%pres, apcc_xpencil, dm, dm%ibcx(:, 4), fl%fbcx_pr(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
    ...
    if( any(dm%ibcx(:, i) == IBC_DIRICHLET) ) then
      a4cc_xpencil = ( qxdx_4cc_xpencil - ONE_THIRD * div_4cc_xpencil) * TWO * mu_4cc_xpencil
    end if
    call Get_x_1st_derivative_C2P_3D(accc_xpencil,  apcc_xpencil, dm, dm%ibcx(:, i), a4cc_xpencil) 
    fl%mx_rhs = fl%mx_rhs + apcc_xpencil
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_dirichlet_geo_bcy(dm%ibcy(:, i), appc_ypencil, fbcy_p4c)
    call Get_y_1st_derivative_P2C_3D(appc_ypencil, apcc_ypencil, dm, dm%ibcy(:, i), fbcy_p4c)
    if(dm%icoordinate == ICYLINDRICAL) call multiple_cylindrical_rn(apcc_ypencil, dm%dpcc, dm%rci, 1, IPENCIL(2))
    mx_rhs_ypencil =  mx_rhs_ypencil + apcc_ypencil
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_dirichlet_geo_bcz(dm%ibcz(:, i), apcp_zpencil, fbcz_pc4)
    call Get_z_1st_derivative_P2C_3D(apcp_zpencil, apcc_zpencil, dm, dm%ibcz(:, i), fbcz_pc4)
    if(dm%icoordinate == ICYLINDRICAL) call multiple_cylindrical_rn(apcc_zpencil, dm%dpcc, dm%rci, 2, IPENCIL(3))
    mx_rhs_zpencil =  mx_rhs_zpencil + apcc_zpencil
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------  
    if ( .not. dm%is_thermo) then
      appc_xpencil = qxiy_ppc_xpencil
      call get_dirichlet_geo_bcx(dm%ibcx(:, i), fl%fbcx_qx * fl%fbcx_qy, fbcx_4pc)
    else
      appc_xpencil = gxiy_ppc_xpencil
      call get_dirichlet_geo_bcx(dm%ibcx(:, i), fl%fbcx_gx * fl%fbcx_qy, fbcx_4pc)
    end if
    
    appc_xpencil = appc_xpencil * qyix_ppc_xpencil
    call Get_x_1st_derivative_P2C_3D( appc_xpencil, acpc_xpencil, dm, dm%ibcx(:, i), fbcx_4pc )
...
!----------------------------------------------------------------------------------------------------------
    ...
    if( any(dm%ibcy(:, i) == IBC_DIRICHLET) ) then
      if(dm%icoordinate == ICYLINDRICAL) then
        if ( .not. dm%is_thermo) then
          ...
          call get_dirichlet_geo_bcy(dm%ibcy(:, i), fl%fbcy_gy * fl%fbcy_qyr, fbcy_c4c)
        end if
      else
        if ( .not. dm%is_thermo) then
          call get_dirichlet_geo_bcy(dm%ibcy(:, i), fl%fbcy_qy * fl%fbcy_qy, fbcy_c4c)
        else
          call get_dirichlet_geo_bcy(dm%ibcy(:, i), fl%fbcy_gy * fl%fbcy_qy, fbcy_c4c)
        end if
      end if
    end if
    ...
    call Get_y_1st_derivative_C2P_3D( accc_ypencil, acpc_ypencil, dm, dm%ibcy(:, i), fbcy_c4c )
...
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      ...
      if ( .not. dm%is_thermo) then
        ...
        call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_qzr * fl%fbcz_qyr, fbcz_cp4)
      else
        acpp_zpencil = gzriy_cpp_zpencil
        call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_gzr * fl%fbcz_qyr, fbcz_cp4)
      end if
    else
      acpp_zpencil1 = qyiz_cpp_zpencil
      if ( .not. dm%is_thermo) then
        acpp_zpencil = qziy_cpp_zpencil
        call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_qz * fl%fbcz_qy, fbcz_cp4)
      else
        acpp_zpencil = gziy_cpp_zpencil
        call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_gz * fl%fbcz_qy, fbcz_cp4)
      end if
    end if
    acpp_zpencil = acpp_zpencil1 * acpp_zpencil
    call Get_z_1st_derivative_P2C_3D( acpp_zpencil, acpc_zpencil, dm, dm%ibcz(:, i), fbcz_cp4 )
...
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      if ( .not. dm%is_thermo) then
        ...
        call get_dirichlet_geo_bcy(dm%ibcy(:, i), fl%fbcy_qzr * fl%fbcy_qzr, fbcy_c4p)
      else
        accc_ypencil = gzriz_ccc_ypencil
        call get_dirichlet_geo_bcy(dm%ibcy(:, i), fl%fbcy_gzr * fl%fbcy_qzr, fbcy_c4p)
      end if
      accc_ypencil = accc_ypencil * qzriz_ccc_ypencil 
      call Get_y_midp_C2P_3D( accc_ypencil, acpc_ypencil, dm, dm%ibcy(:, i), fbcy_c4c)
      my_rhs_ypencil = my_rhs_ypencil + acpc_ypencil
    end if
!----------------------------------------------------------------------------------------------------------
! Y-mom pressure gradient in y direction, Y-pencil, d(sigma_1 p)
!----------------------------------------------------------------------------------------------------------
    call Get_y_1st_derivative_C2P_3D( pres_ypencil, acpc_ypencil, dm, dm%ibcy(:, 4), fl%fbcy_pr(:, :, :) )
...
!----------------------------------------------------------------------------------------------------------
    if(dm%is_thermo .and. (fl%igravity == i .or. fl%igravity == -i) )  then
      call Get_y_midp_C2P_3D(dDens_ypencil, acpc_ypencil, dm, dm%ibcy(:, 5), tm%fbcy_ftp(:, :, :)%d )
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_dirichlet_geo_bcx(dm%ibcx(:, i), appc_xpencil, fbcx_4pc)
    call Get_x_1st_derivative_P2C_3D(appc_xpencil, acpc_xpencil, dm, dm%ibcx(:, i), fbcx_4pc)
    fl%my_rhs = fl%my_rhs + acpc_xpencil
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      ...
      if( any(dm%ibcy(:, i) == IBC_DIRICHLET) ) &
      ac4c_ypencil1 = qyrdy_c4c_ypencil
    else
      accc_ypencil1 = qydy_ccc_ypencil
      if( any(dm%ibcy(:, i) == IBC_DIRICHLET) ) &
      ac4c_ypencil1 = qydy_c4c_ypencil
    end if
    
    accc_ypencil = ( accc_ypencil1 - ONE_THIRD * div_ccc_ypencil) * TWO * mu_ccc_ypencil
    if( any(dm%ibcy(:, i) == IBC_DIRICHLET) ) &
    ac4c_ypencil = ( ac4c_ypencil1 - ONE_THIRD * div_c4c_ypencil) * TWO * mu_c4c_ypencil
    if(dm%icoordinate == ICYLINDRICAL) then
      call multiple_cylindrical_rn(accc_ypencil, dm%dccc, ONE/dm%rci, 1, IPENCIL(2))
      if( any(dm%ibcy(:, i) == IBC_DIRICHLET) ) &
      call multiple_cylindrical_rn_x4x(ac4c_ypencil, dm%dcpc, ONE/dm%rpi, 1, IPENCIL(2))
    end if
    call Get_y_1st_derivative_C2P_3D(accc_ypencil,  acpc_ypencil, dm, dm%ibcy(:, 2), ac4c_ypencil) 
    my_rhs_ypencil = my_rhs_ypencil + acpc_ypencil
!----------------------------------------------------------------------------------------------------------
! Y-mom diffusion term (y-v-3/4), Z-pencil, d_z [mu^{y,z} * (dy(qz/r) + 1/r dz(qr/r) - 1/r * (qz/r)^r)] at (i, j', k)
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      acpp_zpencil  = qzrdy_cpp_zpencil
      acpp_zpencil1 = qyrdz_cpp_zpencil
      call multiple_cylindrical_rn(acpp_zpencil1, dm%dcpp, dm%rpi, 1, IPENCIL(3))
      acpp_zpencil = acpp_zpencil + acpp_zpencil1
      acpp_zpencil1 = qzriy_cpp_zpencil
      call multiple_cylindrical_rn(acpp_zpencil1, dm%dcpp, dm%rpi, 1, IPENCIL(3))
      acpp_zpencil = acpp_zpencil - acpp_zpencil1
    else
    ...
    call get_dirichlet_geo_bcz(dm%ibcz(:, i), acpp_zpencil, fbcz_cp4)
    call Get_z_1st_derivative_P2C_3D(acpp_zpencil, acpc_zpencil, dm, dm%ibcz(:, i), fbcz_cp4)
    my_rhs_zpencil =  my_rhs_zpencil + acpc_zpencil
!----------------------------------------------------------------------------------------------------------
! Y-mom diffusion term (y-v-4/4), Y-pencil, ( -2 * mu * [ 1/r^2 * dzqz - 1/3 div + 1/r * (qr/r)^r ] )^r at (i, j', k)
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      accc_ypencil = qzdz_ccc_ypencil
      call multiple_cylindrical_rn(accc_ypencil, dm%dccc, dm%rci, 2, IPENCIL(2))
      if( any(dm%ibcy(:, 3) == IBC_DIRICHLET) ) then
        ac4c_ypencil = qzdz_c4c_ypencil
        call multiple_cylindrical_rn_x4x(ac4c_ypencil, dm%dcpc, dm%rpi, 2, IPENCIL(2))
      end if

      accc_ypencil1 = qyriy_ccc_ypencil
      call multiple_cylindrical_rn(accc_ypencil1, dm%dccc, dm%rci, 1, IPENCIL(2))
      if( any(dm%ibcy(:, 2) == IBC_DIRICHLET) ) then
        ac4c_ypencil1 = fl%fbcy_qyr(:, :, :)    
        call multiple_cylindrical_rn_x4x(ac4c_ypencil1, dm%dcpc, dm%rpi, 1, IPENCIL(2))
      end if

      accc_ypencil = (accc_ypencil - ONE_THIRD * div_ccc_ypencil + accc_ypencil1) * TWO * mu_ccc_ypencil
      if( any(dm%ibcy(:, 2) == IBC_DIRICHLET) ) then
        ac4c_ypencil = (ac4c_ypencil - ONE_THIRD * div_c4c_ypencil + ac4c_ypencil1) * TWO * mu_c4c_ypencil
      end if

      call Get_y_midp_C2P_3D(accc_ypencil, acpc_ypencil, dm, dm%ibcy(:, i), ac4c_ypencil)
      my_rhs_ypencil =  my_rhs_ypencil - acpc_ypencil
    end if
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------  
    ...
    call get_dirichlet_geo_bcx(dm%ibcx(:, i), apcp_xpencil, fbcx_4cp)
    call Get_x_1st_derivative_P2C_3D( appc_xpencil, accp_xpencil, dm, dm%ibcx(:, i), fbcx_4cp )
    fl%mz_rhs = fl%mz_rhs - accp_xpencil
!----------------------------------------------------------------------------------------------------------
! Y-mom convection term (y-c2/4), Y-pencil: -d(<gy>^z * <qz/r>^r)/dy at (i, j, k')
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
    ...
    call get_dirichlet_geo_bcy(dm%ibcy(:, i), acpp_ypencil, fbcy_c4p)
    call Get_y_1st_derivative_P2C_3D( acpp_ypencil, accp_ypencil, dm, dm%ibcy(:, i), fbcy_c4p )
    mz_rhs_ypencil = mz_rhs_ypencil - accp_ypencil
!----------------------------------------------------------------------------------------------------------
! Z-mom convection term (z-c3/4), Z-pencil: -d(<gz>^z * <qz>^z)/dz at (i, j, k')
!----------------------------------------------------------------------------------------------------------
    if ( .not. dm%is_thermo) then
      accc_zpencil = qziz_ccc_zpencil
      call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_qz * fl%fbcz_qz, fbcz_cc4)
    else
      accc_zpencil = gziz_ccc_zpencil
      call get_dirichlet_geo_bcz(dm%ibcz(:, i), fl%fbcz_gz * fl%fbcz_qz, fbcz_cc4)
    end if

    accc_zpencil = accc_zpencil * qziz_ccc_zpencil

    call Get_z_1st_derivative_C2P_3D( accc_zpencil, accp_zpencil, dm, dm%ibcz(:, i), fbcz_cc4 )
...
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      ...
      call get_dirichlet_geo_bcy(dm%ibcy(:, i), acpp_ypencil, fbcy_c4p)
      call Get_y_midp_C2P_3D( acpp_ypencil, accp_ypencil, dm, dm%ibcy(:, i), fbcy_c4p)
      mz_rhs_ypencil = mz_rhs_ypencil - accp_ypencil
    end if
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------
    if(dm%is_thermo .and. (fl%igravity == i .or. fl%igravity == -i) )  then
      call Get_z_midp_C2P_3D(dDens_zpencil, accp_zpencil, dm, dm%ibcz(:, 5), tm%fbcz_ftp(:, :, :)%d )
...
!----------------------------------------------------------------------------------------------------------
    ...
    call get_dirichlet_geo_bcx(dm%ibcx(:, i), apcp_xpencil, fbcx_4cp)
    call Get_x_1st_derivative_P2C_3D(apcp_xpencil, accp_xpencil, dm, dm%ibcx(:, i), fbcx_4cp)
    fl%mz_rhs = fl%mz_rhs + accp_xpencil
!----------------------------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      ...
      if( any(dm%ibcz(:, i) == IBC_DIRICHLET) ) then
        acc4_zpencil = qzdz_cc4_zpencil
        call multiple_cylindrical_rn_xx4(acc4_zpencil, dm%dccp, dm%rci, 2, IPENCIL(3))
      end if

      accc_zpencil1 = qyriy_ccc_zpencil
      call multiple_cylindrical_rn(accc_zpencil1, dm%dccc, dm%rci, 1, IPENCIL(3))
      if( any(dm%ibcz(:, i) == IBC_DIRICHLET) ) then
        acc4_zpencil1 = fl%fbcz_qyr(:, :, :)
        call multiple_cylindrical_rn_xx4(acc4_zpencil1, dm%dccp, dm%rci, 1, IPENCIL(3))
      end if
    else
      accc_zpencil  = qzdz_ccc_zpencil
      accc_zpencil1 = ZERO
    ...
    if( any(dm%ibcz(:, i) == IBC_DIRICHLET) ) then
      acc4_zpencil = ( acc4_zpencil - ONE_THIRD * div_cc4_zpencil + acc4_zpencil1) * TWO * mu_cc4_zpencil
    end if
    
    call Get_z_1st_derivative_C2P_3D(accc_zpencil, accp_zpencil, dm, dm%ibcz(:, i), acc4_zpencil) 
    mz_rhs_zpencil = mz_rhs_zpencil + accp_zpencil

!----------------------------------------------------------------------------------------------------------
! Z-mom diffusion term (z-v-4/4), Y-pencil, (mu^{y,z} * [ dy d(qz/r) + 1/r^2 * dz d(qy) - 1/r * (qz/r)^y ] )^r at (i, j, k')
!---------------------------------------------------------------------------------------------------------
    if(dm%icoordinate == ICYLINDRICAL) then
      acpp_ypencil = qzrdy_cpp_ypencil
...
!==========================================================================================================
    if (fl%idriven == IDRVF_X_MASSFLUX) then
      ...
      !call Get_volumetric_average_3d(.false., dm%ibcy(:, 1), fl%fbcy_var(:, :, :, 1), dm, dm%dpcc, fl%mx_rhs, rhsx_bulk, "mx_rhs")
      ...
      !call Get_volumetric_average_3d(.false., dm%ibcy(:, 3), fl%fbcy_var(:, :, :, 3), dm, dm%dccp, fl%mz_rhs, rhsz_bulk, "mz_rhs")
