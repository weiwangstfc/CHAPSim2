module boundary_conditions_mod
  ...
  public  :: update_bc_interface_vof
...
! enthalpy-
    if(dm%is_thermo) then
      do n = 1, 2
        dm%fbcx_t(n, :, :) = dm%fbcx_const(n, 5)
        dm%fbcy_t(:, n, :) = dm%fbcy_const(n, 5)
        dm%fbcz_t(:, :, n) = dm%fbcz_const(n, 5)
      end do
      do n = 3, 4
        dm%fbcx_t(n, :, :) = dm%fbcx_const(n - 2, 5)
        dm%fbcy_t(:, n, :) = dm%fbcy_const(n - 2, 5)
        dm%fbcz_t(:, :, n) = dm%fbcz_const(n - 2, 5)
      end do
    end if
...
! vof-
    if(dm%is_vof) then
      do n = 1, 2
        dm%fbcx_vof(n, :, :) = dm%fbcx_const(n, 6)
        dm%fbcy_vof(:, n, :) = dm%fbcy_const(n, 6)
        dm%fbcz_vof(:, :, n) = dm%fbcz_const(n, 6)
      end do
      do n = 3, 4
        dm%fbcx_vof(n, :, :) = dm%fbcx_const(n - 2, 6)
        dm%fbcy_vof(:, n, :) = dm%fbcy_const(n - 2, 6)
        dm%fbcz_vof(:, :, n) = dm%fbcz_const(n - 2, 6)
      end do
    end if
...
!----------------------------------------------------------------------------------------------------------
    ...
    if(dm%is_thermo) then
      ...
      allocate( dm%fbcx_t(             4, dm%dccc%xsz(2), dm%dccc%xsz(3)) )! default x pencil
      allocate( dm%fbcy_t(dm%dccc%ysz(1),              4, dm%dccc%ysz(3)) )! default y pencil
      allocate( dm%fbcz_t(dm%dccc%zsz(1), dm%dccc%zsz(2),             4)  )! default z pencil

      allocate( dm%ftpbcx_var(             4, dm%dccc%xsz(2), dm%dccc%xsz(3)) )! default x pencil
      allocate( dm%ftpbcy_var(dm%dccc%ysz(1),              4, dm%dccc%ysz(3)) )! default y pencil
      allocate( dm%ftpbcz_var(dm%dccc%zsz(1), dm%dccc%zsz(2),             4)  )! default z pencil
    end if

    if(dm%is_vof) then
      allocate( dm%fbcx_vof(             4, dm%dccc%xsz(2), dm%dccc%xsz(3)) )! default x pencil
      allocate( dm%fbcy_vof(dm%dccc%ysz(1),              4, dm%dccc%ysz(3)) )! default y pencil
      allocate( dm%fbcz_vof(dm%dccc%zsz(1), dm%dccc%zsz(2),             4)  )! default z pencil
    end if
...
!==========================================================================================================
  subroutine update_flow_bc_1dm_halo(dm, fl) ! 
    ...
    if(dm%ibcz(1, 3) == IBC_DIRICHLET ) then
      ...
      accp_zpencil(:, :, 1) = dm%fbcz_const(1, 3)
    ...
    if(dm%ibcz(2, 3) == IBC_DIRICHLET ) then
      ...
      call transpose_x_to_y(fl%qz, accp_ypencil, dm%dccp)
      call transpose_y_to_z(accp_ypencil, accp_zpencil, dm%dccp)
      accp_zpencil(:, :, dm%dccp%zsz(3)) = dm%fbcz_const(2, 3)
      call transpose_z_to_y(accp_zpencil, accp_ypencil, dm%dccp)
      call transpose_y_to_x(accp_ypencil, fl%qz, dm%dccp)     
    end if
    !-----------uz at x-bc-----------
...
!   end subroutine
!==========================================================================================================
  subroutine update_bc_interface_vof(dm0, vf0, dm1, vf1)
    use parameters_constant_mod
    use udf_type_mod
    implicit none
    type(t_domain), intent(inout) :: dm0, dm1
    type(t_vof), intent(in)       :: vf0, vf1
...
!----------------------------------------------------------------------------------------------------------
    ...
    m = 6
    if(dm1%ibcx(1, m) == IBC_INTERIOR) then
      dm1%fbcx_vof(1, :, :) = vf0%phi(dm0%nc(1),     :, :)
      dm1%fbcx_vof(3, :, :) = vf0%phi(dm0%nc(1) - 1, :, :)
    end if
    if(dm0%ibcx(2, m) == IBC_INTERIOR) then
      dm0%fbcx_vof(2, :, :) = vf1%phi(1, :, :)
      dm0%fbcx_vof(4, :, :) = vf1%phi(2, :, :)
    end if
!----------------------------------------------------------------------------------------------------------
!   bc in y - direction
!----------------------------------------------------------------------------------------------------------
    ...
    m = 6
    if(dm1%ibcy(1, m) == IBC_INTERIOR) then
      dm1%fbcy_vof(:, 1, :) = vf0%phi(:, dm0%nc(1),     :)
      dm1%fbcy_vof(:, 3, :) = vf0%phi(:, dm0%nc(1) - 1, :)
    end if
    if(dm0%ibcy(2, m) == IBC_INTERIOR) then
      dm0%fbcy_vof(:, 2, :) = vf1%phi(:, 1, :)
      dm0%fbcy_vof(:, 4, :) = vf1%phi(:, 2, :)
    end if
!----------------------------------------------------------------------------------------------------------
!   bc in z - direction
!----------------------------------------------------------------------------------------------------------
    ...
    m = 6
    if(dm1%ibcz(1, m) == IBC_INTERIOR) then
      dm1%fbcz_vof(:, :, 1) = vf0%phi(:, :, dm0%nc(1)    )
      dm1%fbcz_vof(:, :, 3) = vf0%phi(:, :, dm0%nc(1) - 1)
    end if
    if(dm0%ibcz(2, m) == IBC_INTERIOR) then
      dm0%fbcz_vof(:, :, 2) = vf1%phi(:, :, 1)
      dm0%fbcz_vof(:, :, 4) = vf1%phi(:, :, 2)
    end if
