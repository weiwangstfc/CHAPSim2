...
!_______________________________________________________________________________
  ...
  complex(wp), allocatable, dimension(:, :, :) :: cwz
!_______________________________________________________________________________
! FFT library only needs to be initialised once
!_______________________________________________________________________________
...
!_______________________________________________________________________________
    allocate ( ax(nx) ); ax = ZERO
    allocate ( bx(nx) ); bx = ZERO

    allocate ( ay(ny) ); ay = ZERO
    allocate ( by(ny) ); by = ZERO

    allocate ( az(nz) ); az = ZERO
    allocate ( bz(nz) ); bz = ZERO

    call Calculate_sine_cosine_unit(ax, bx, nx, bcx)
    call Calculate_sine_cosine_unit(ay, by, ny, bcy)
    call Calculate_sine_cosine_unit(az, bz, nz, bcz)
!_______________________________________________________________________________
...
!_______________________________________________________________________________
    ...
    allocate ( cwz( sp%xst(1) : sp%xen(1), &
                    sp%xst(2) : sp%xen(2), &
                    sp%xst(3) : sp%xen(3)) )
...
!_______________________________________________________________________________
    ...
    call Transform_2nd_derivative_spectral_1d &
          (d%bc(1, 1), sp%xst(1), sp%xen(1), nx, d%h(1), t2x)
    call Transform_2nd_derivative_spectral_1d &
          (d%bc(1, 2), sp%xst(2), sp%xen(2), ny, d%h(2), t2y)
    call Transform_2nd_derivative_spectral_1d &
          (d%bc(1, 3), sp%xst(3), sp%xen(3), nz, d%h(3), t2z)
...
!===============================================================================
  subroutine Transform_2nd_derivative_spectral_1d(ibc, n0, n1, nn, dd, t2)
    ...
    if(method == 1) then
      ...
      do i = n0, n1
        w = angle * REAL(i  - 1, WP) / REAL(nn, WP) !
      ...
      do i = n0, n1
        w = angle * REAL(i - 1, WP) / REAL(nn, WP) ! check, for 0-n/2, pi or 2pi?
        cosw = cos_wp(w)
...
! compute r2c transform, forward FFT
!_______________________________________________________________________________
    call decomp_2d_fft_3d(rhsx,cwz)
!_______________________________________________________________________________
! fft normalisation
!_______________________________________________________________________________
    cwz = cwz / real(nx, kind=wp) / &
                real(ny, kind=wp) / &
                real(nz, kind=wp)
!_______________________________________________________________________________
    ...
    do k = sp%xst(3),sp%xen(3)
      do j = sp%xst(2),sp%xen(2)
        ...
        do i = sp%xst(1) + 1, sp%xen(1)
          cwRe1 = real ( cwz(i, j, k), wp )
          cwIm1 = aimag( cwz(i, j, k) )
          cwRe2 = real ( cwz(sp%xen(1) - i + 2, j, k), wp )
          cwIm2 = aimag( cwz(sp%xen(1) - i + 2, j, k) )
    ...
    do k = sp%xst(3),sp%xen(3)
      do j = sp%xst(2),sp%xen(2)
        ...
        do i = sp%xst(1) + 1, sp%xen(1)
          ...
          cwz(i, j, k) = cmplx( bRe1 - aIm1 + aRe2 + bIm2, &
...
! compute c2r transform, inverse FFT
!_______________________________________________________________________________
    call decomp_2d_fft_3d(cwz,rhsx)
!_______________________________________________________________________________
